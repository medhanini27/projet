...
void afficher_date(GtkWidget *liste)
{
	...
	f = fopen("/home/med/Desktop/projet/src/dateD.txt", "r");
	...
	{ f = fopen("/home/med/Desktop/projet/src/dateD.txt", "a+");
		...
		{
              while(fscanf(f,"%s %s %s %s %d %s %s \n",jour,mois,anne,heure,&r,id,salle)!=EOF)
		{if(strcmp(id,d)==0)
		{
	gtk_list_store_append (store, &iter);
	gtk_list_store_set (store, &iter, JOUR, jour, MOIS, mois, ANNE, anne,HEURE,heure,SALLE,salle, -1); 
		}
		}
		}
		fclose(f);
